...
if(($_SERVER['REQUEST_METHOD']??'GET')==='POST'){
  ...
  if($idx===null){$error='Matéria não encontrada.';} else {
    ...
    if($action==='confirm'){
      ...
      $log[]=['id'=>uniqid('valid_'),'news_id'=>$id,'title'=>$title,'action'=>'REVISAO_SOLICITADA','reason'=>cv_reason($news[$idx]),'created_at'=>$now]; $notice='Revisão editorial solicitada.';
...
?><!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Validade Editorial | TV Sumaré</title><link rel="stylesheet" href="admin.css?v=2.0.3"></head><body><div class="admin"><?php include __DIR__.'/_menu.php'; ?><main class="main"><div class="top"><div><span class="eyebrow">Governança Editorial</span><h1>Validade das Matérias</h1><p class="muted">Conteúdo temporal entra em revisão após 7 dias; demais matérias após 30 dias. Nada é removido automaticamente.</p></div></div><?php if($notice):?><div class="notice"><?=cv_h($notice)?></div><?php endif;?><?php if($error):?><div class="notice error"><?=cv_h($error)?></div><?php endif;?><div class="admin-kpi-grid"><div class="admin-kpi"><span>Publicadas</span><strong><?=count($news)?></strong></div><div class="admin-kpi"><span>Pedem checagem</span><strong><?=count($review)?></strong></div><div class="admin-kpi"><span>Registros de validade</span><strong><?=count($log)?></strong></div></div><section class="box"><h2>Fila de checagem</h2><?php if(!$review):?><p class="muted">Nenhuma matéria exige checagem neste momento.</p><?php else:?><div class="table-wrap"><table><thead><tr><th>Matéria</th><th>Idade</th><th>Motivo</th><th>Ação editorial</th></tr></thead><tbody><?php foreach($review as $n):?><tr><td><strong><?=cv_h($n['title']??'Sem título')?></strong><br><small><?=cv_h(($n['city']??'Região').' • '.($n['category']??'Notícia'))?></small></td><td><?=cv_h((cv_age($n)===null?'sem data':cv_age($n).' dias'))?></td><td><?=cv_h(cv_reason($n))?></td><td><div class="actions"><form method="post"><?=tvs_csrf_field()?><input type="hidden" name="id" value="<?=cv_h($n['id']??'')?>"><button class="btn secondary" name="action" value="confirm">Confirmar atual</button></form><form method="post"><?=tvs_csrf_field()?><input type="hidden" name="id" value="<?=cv_h($n['id']??'')?>"><button class="btn" name="action" value="review">Solicitar revisão</button></form><form method="post" onsubmit="return confirm('Arquivar esta matéria por perda de validade? Ela irá para a Lixeira e poderá ser restaurada.');"><?=tvs_csrf_field()?><input type="hidden" name="id" value="<?=cv_h($n['id']??'')?>"><button class="btn danger" name="action" value="archive">Arquivar</button></form></div></td></tr><?php endforeach;?></tbody></table></div><?php endif;?></section><section class="box" style="margin-top:16px"><h2>Notificação ao Editor-Chefe</h2><p class="muted">A fila acima é o alerta operacional interno. O verificador automático gera <code>data/content_validity_alerts.json</code> para integração com e-mail e, quando disponível/configurado, WhatsApp. A integração externa não remove nem arquiva conteúdo automaticamente.</p></section></main></div></body></html>
