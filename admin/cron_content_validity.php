...
function cvc_date($n){foreach(['published_at','updated_at','created_at','date'] as $k){if(!empty($n[$k])){$ts=strtotime((string)$n[$k]);if($ts)return $ts;}}return 0;}
...
foreach($news as $n){$ts=cvc_date($n);$age=$ts?max(0,(int)floor(($now-$ts)/86400)):null;$limit=cvc_sensitive($n)?7:30;$checked=strtotime((string)($n['validity_checked_at']??''));if($checked && ($now-$checked)<($limit*86400))continue;if($age===null||$age>=$limit){$alerts[]=['id'=>'alert_'.substr(hash('sha256',(string)($n['id']??'').($n['title']??'')),0,16),'news_id'=>$n['id']??'','title'=>$n['title']??'Sem título','city'=>$n['city']??'Região','category'=>$n['category']??'Notícia','age_days'=>$age,'priority'=>cvc_sensitive($n)?'alta':'normal','reason'=>$age===null?'Data editorial não identificada.':"Conteúdo com {$age} dia(s) requer conferência editorial.",'status'=>'aguardando_revisao','notification_channels'=>['painel','email_quando_configurado','whatsapp_quando_configurado'],'detected_at'=>date('c')];}}
