...
$ready=array_values(array_filter($videos,function($v) use ($queue){ if(!ds_social_ready($v)) return false; foreach($queue as $q){ if(($q['video_id']??'')===($v['id']??'') && ds_queue_active($q)) return false; } return true; }));
