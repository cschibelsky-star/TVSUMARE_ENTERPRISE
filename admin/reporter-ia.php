...
function rpia_generate_script($news){
  ...
  if($anthropic!=='' && function_exists('curl_init')){
    ...
    $outboundOptions=tvs_outbound_curl_options($url,25);
    if($outboundOptions!==null){
      $ch=curl_init($url);
      curl_setopt_array($ch,$outboundOptions+[CURLOPT_POST=>true,CURLOPT_POSTFIELDS=>$payload,CURLOPT_RETURNTRANSFER=>true,CURLOPT_HTTPHEADER=>['Content-Type: application/json','x-api-key: '.$anthropic,'anthropic-version: 2023-06-01']]);
...
function rpia_job_orientation($job,$cfg){
  $jobOrientation=trim((string)($job['orientation']??''));
  if(in_array($jobOrientation,['landscape','portrait'],true)) return $jobOrientation;
  $cfgOrientation=trim((string)($cfg['heygen_orientation']??'landscape'));
  return in_array($cfgOrientation,['landscape','portrait'],true)?$cfgOrientation:'landscape';
}
function rpia_video_agent_prompt($job,$cfg){
  $jobOrientation=rpia_job_orientation($job,$cfg);
  $orientation=$jobOrientation==='portrait'?'vertical/portrait 9:16':'landscape 16:9';
  $durationTarget=trim((string)($job['duration_target']??'45 a 60 segundos'));
  if($durationTarget==='') $durationTarget='45 a 60 segundos';
  ...
  return "Crie um vídeo jornalístico regional para a {$brand}\n\n".
    "Formato: {$orientation}. Duração alvo: {$durationTarget}. Tom: apresentador de telejornal regional, claro, confiável, objetivo e sem sensacionalismo.\n\n".
...
function rpia_heygen_create($job,$cfg){
  ...
  $orientation=rpia_job_orientation($job,$cfg);
  $payload=['prompt'=>rpia_video_agent_prompt($job,$cfg),'mode'=>'generate','incognito_mode'=>rpia_bool($cfg['heygen_incognito_mode']??'0'),'orientation'=>$orientation];
  ...
  // Boletins criados pelo módulo Boletim IA carregam orientation=portrait e sempre sobrescrevem a orientação geral.
...
if(($_SERVER['REQUEST_METHOD']??'GET')==='POST'){
  ...
  if($action==='send_heygen'){
    $idx=null; [$job,$jobs]=rpia_find_videojob($_POST['job_id']??'',$idx);
    if(!$job) $err='Roteiro não encontrado.'; else { $r=rpia_heygen_create($job,$cfg); if(!$r['ok']) $err=$r['error']; else { $jobs[$idx]['heygen_session_id']=$r['session_id']; if(!empty($r['video_id'])) $jobs[$idx]['heygen_video_id']=$r['video_id']; $jobs[$idx]['heygen_status']=$r['status']; $jobs[$idx]['heygen_callback_id']=$job['id']; $jobs[$idx]['heygen_payload']=$r['payload']; $jobs[$idx]['status']='heygen_agente_processando'; $jobs[$idx]['updated_at']=date('c'); rpia_write('videos_ia.json',$jobs); $msg='Agente de Vídeo HeyGen criado. Formato: '.($r['payload']['orientation']??'padrão').'. Sessão: '.($r['session_id']?:'sem sessão').' Vídeo: '.($r['video_id']?:'aguardando video_id'); } }
  }
