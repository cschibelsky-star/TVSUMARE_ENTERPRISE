...
if($_SERVER['REQUEST_METHOD']==='POST'){
  ...
  if($action==='send_veo'){
    $idx=null; $jobs=null; $job=tvp_find_job($_POST['job_id']??'',$jobs,$idx);
    if(!$job) tvp_admin_redirect(['err'=>'Job não encontrado.']);
    if(trim((string)($job['script']??''))==='') tvp_admin_redirect(['err'=>'Gere e aprove o roteiro antes de enviar para VEO.']);
    if(($job['status']??'')!=='roteiro_aprovado') tvp_admin_redirect(['err'=>'Salve/aprove o roteiro antes de enviar para VEO.']);
    $r=tvp_send_veo($job);
    if(!$r['ok']) tvp_admin_redirect(['err'=>$r['error']??'Falha no envio para VEO.']);
    $jobs[$idx]['video_engine']='veo'; $jobs[$idx]['veo_operations']=$r['operations']??[]; $jobs[$idx]['status']='gerando'; $jobs[$idx]['updated_at']=date('c'); tvp_save_video_jobs($jobs);
    tvp_admin_redirect(['msg'=>'VEO iniciou 3 cenas de vídeo. Use Atualizar VEO até a composição ficar pronta.']);
  }
  if($action==='check_veo'){
    $idx=null; $jobs=null; $job=tvp_find_job($_POST['job_id']??'',$jobs,$idx);
    if(!$job) tvp_admin_redirect(['err'=>'Job não encontrado.']);
    ...
    tvp_admin_redirect(['msg'=>!empty($r['video_url'])?'Vídeo VEO finalizado e pronto para publicação.':'VEO ainda está processando.']);
  }
...
<div class="workflow"><span class="step <?=in_array($status,['sugerido','roteiro_pendente','roteiro_revisao','roteiro_aprovado','gerando','pronto','publicado'],true)?'done':''?>">1 Fila</span><span class="step <?=in_array($status,['roteiro_revisao','roteiro_aprovado','gerando','pronto','publicado'],true)?'done':($status==='roteiro_pendente'?'on':'')?>">2 Roteiro</span><span class="step <?=in_array($status,['roteiro_aprovado','gerando','pronto','publicado'],true)?'done':($status==='roteiro_revisao'?'on':'')?>">3 Aprovação</span><span class="step <?=in_array($status,['gerando','pronto','publicado'],true)?'done':($status==='roteiro_aprovado'?'on':'')?>">4 <?=tvp_h($engine==='heygen'?'HeyGen':'VEO')?></span><span class="step <?=in_array($status,['pronto','publicado'],true)?'done':($status==='gerando'?'on':'')?>">5 Pronto</span><span class="step <?=$status==='publicado'?'done':''?>">6 Publicado</span></div>
