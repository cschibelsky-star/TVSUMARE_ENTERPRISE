...
$requiredMarkers=[
    ...
    'Social reviewed copy'=>"$engine=($caption!=='' && $hashtags!=='')?'revisado':'automatico';",
...
if(strpos($code,"network_status'] as $net=>$st")!==false){
    echo "Social status UI: status por rede já aplicado.\n";
    echo "SOCIAL_DISTRIBUTION_HARDENING_APPLIED=SIM\n";
    exit(0);
}

$old="<strong><?=ds_h(ds_status_label($q['status']??''))?></strong>";
$new="<strong><?=ds_h(ds_status_label($q['status']??''))?></strong><?php if(!empty($q['network_status'])): ?><br><small><?php foreach($q['network_status'] as $net=>$st): ?><?=ds_h(ucfirst($net))?>: <?=ds_h(ds_status_label($st))?> <?php endforeach; ?></small><?php endif; ?>";
